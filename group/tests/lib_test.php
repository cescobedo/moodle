...
class core_group_lib_testcase extends advanced_testcase {
    ...
    public function test_groups_add_member_conversation() {
        ...
        $this->assertEquals(
                2,
                $DB->count_records_sql("SELECT COUNT(cm.id)
                                          FROM {message_conversation_members} cm
                                          JOIN {message_conversation_area} ca ON cm.conversationid = ca.conversationid
                                         WHERE ca.contextid = ?
                                               AND ca.component = 'core_group'
                                               AND ca.itemtype = 'groups'", [$coursecontext1->id])
        );
        // Check if the user has been added to the conversation when the conversation area is disabled.
        ...
        $this->assertEquals(
                ...
                $DB->count_records_sql("SELECT COUNT(cm.id)
                                          FROM {message_conversation_members} cm
                                         WHERE cm.conversationid = ?", [$conversationarea->conversationid])
        );
    }
    ...
    public function test_groups_remove_member_conversation() {
        ...
        // Check if the user has been removed to the conversation when the conversation area is disabled.
        $conversationarea = \core_message\api::get_conversation_area('core_group',
                                                                     'groups',
                                                                      $group1->id,
                                                                      $coursecontext1->id);
     ...
     */
    public function test_add_members_group_updated_conversation_area_enabled() {
        global $DB;
        ...
        $this->getDataGenerator()->create_group_member(array('groupid' => $group1->id, 'userid' => $user2->id));
        $this->getDataGenerator()->create_group_member(array('groupid' => $group1->id, 'userid' => $user3->id));
        // Check there aren't members in the conversation.
        $this->assertEquals(
                0,
                $DB->count_records_sql("SELECT COUNT(cm.id)
                                          FROM {message_conversation_members} cm
