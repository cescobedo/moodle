...
if (has_capability('moodle/contentbank:useeditor', $context)) {
    ...
    if (!empty($editabletypes)) {
        ...
        $toolbar[] = ['name' => get_string('add'), 'link' => $editbaseurl, 'dropdown' => true, 'contenttypes' => $editabletypes,  'action' => 'add'];
    }
}
...
if (has_capability('moodle/contentbank:upload', $context)) {
    ...
    if (!empty($accepted)) {
        ...
        $toolbar[] = ['name' => get_string('upload', 'contentbank'), 'link' => $importurl, 'icon' => 'i/upload', 'action' => 'upload'];
    }
}
